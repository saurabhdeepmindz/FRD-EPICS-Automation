...
- **PR 3:** WC-15 (lean references-notes injection into chat) ✅ · WC-20 (register unit spec + full regression sweep: tsc green both apps, 36 backend tests pass, all services healthy, existing pages render) ✅.
- **Deferred (follow-ups):** WC-14 (screenshot-to-chat vision attach) · full WC-15 references **tab** (URL/document ingestion + RAG, à la v11 Track RR) · chat-stream token proxy · Playwright E2E journey.

## ✅ v12 Wireframe Copilot — core sprint COMPLETE (PR 1 backend · PR 2 frontend · PR 3 references+verify). Conversational AI + screen-routed change register with status/trail/before-after/accept-revert, feedback-document ingestion, Design-System grounding, export + re-apply hook. Verified e2e on TestDemoProject; strictly additive.
...
- [x] **WC-01 (DB): copilot + change-register models + migration** (P0, L) — ✅ done (5 tables + 3 enums in `new_prd_generator`; client regen; backend boots clean; tsc green)
  ...
  - `BaWireframeFeedbackImport` (`id`, `projectId`, `threadId?`, `fileName?`, `source` CUSTOMER|INTERNAL, **`uploadedBy String?`**, **`uploadedAt DateTime?`** (auto/editable), `rawText @db.Text?`, `parsedCount Int @default(0)`, `createdAt`; `@@index([projectId])`). `BaWireframeChange` gets a nullable **`importId`** so every change traces to its source document/batch.
...
- [ ] **WC-21 (BE+AI): feedback-document ingestion + screen-wise routing** (P0, L)
  ...
  - `WireframeFeedbackService` (BE): accept an **uploaded document** (`TextExtractionService` — PDF/DOCX/MD/TXT, reused from RR) **or pasted text**; call the parser; **resolve `screenRef` → slug** (fuzzy match vs the current lo-fi/hi-fi set; e.g. "Screen 01 — Landing" → `screen-01-landing`); classify scope + phase; return a **staging payload** (nothing persisted). Single-screen paste = one block, same path. On confirm, records a **`BaWireframeFeedbackImport`** (fileName, source, **uploadedBy**, **uploadedAt** auto/editable) and links each created change's **`importId`**. Endpoint: `POST .../copilot/feedback` (multipart or text).

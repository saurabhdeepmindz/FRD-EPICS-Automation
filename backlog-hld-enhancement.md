...
|----|-----|------|------|--------------|
| ✅ HD-01 | P1 | L | **DONE (2026-06-07)** — Streaming chat (SSE): ai `/hld-chat-stream` (anthropic/openai token streams; gemini one-shot) → NestJS SSE proxy (`/copilot/chat-stream`, forwards deltas + persists the final turn) → frontend live token rendering (`streamHldCopilotChat` + a live accordion item with a cursor), with automatic fallback to the non-streaming path on any error. | Token-by-token answers across FastAPI → NestJS → frontend. |
| ✅ HD-02 | P1 | L | **Curated architecture-pattern catalog** — DONE (2026-06-07). Expanded `BUILTIN_ARCH_TEMPLATES` to 13 curated patterns (multi-tenant RLS, schema-per-tenant, event-driven, layered 3-tier, RAG/AI, CQRS+ES, microservices, modular-monolith, API-gateway+BFF, serverless, hexagonal/clean, strangler-fig, observability) via the same `TemplateSource`/`HldTemplate` contract. | Reuse-first honoured; no rework. |
| ✅ HD-03 | P2 | S | **Gemini provider** — DONE (2026-06-07). Added the Gemini code path in ai-service `_complete_text` (lazy `google-generativeai` import; `GEMINI_API_KEY`/`GEMINI_MODEL` config; added to requirements). Stays gated off in the model picker until a key is set, then works drop-in. | Untestable here (no key); path implemented + key-gated. |
...
| ✅ HD-10 | P1 | L | **DONE (2026-06-07)** — HLD repository + RAG similarity + browse: every finalized HLD is indexed into a central repository; embed sections/diagrams into a vector store; "Find similar HLDs" (RAG) surfaces related past designs inside the Copilot, plus a standard **browse/search** UI to explore other HLDs (filter by domain/stack/pattern). | High-value knowledge reuse; needs an embeddings provider + vector store — scope after MVP. Feeds the Copilot's research with *your own* prior HLDs. |
